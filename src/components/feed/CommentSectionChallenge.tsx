...
import { addCommentChallenge, likeComment, getCommentLikes } from '@/actions/interactions/interaction';
...
interface CommentSectionProps {
  ...
  export default function CommentSectionChallenge({ 
    challengeId,
    comments,
    currentPlaybackTime,
    onCommentAdded,
    ...
    const handleAddComment = async (e: React.FormEvent) => {
      ...
      try {
        ...
        const { error, data } = await addCommentChallenge(
          content.trim(), 
  ...
  const handleLikeComment = async (commentId: string, challengeId: string) => {
    if (!user) {
      toast({
        title: "Authentification requise",
    ...
    try {
      ...
      const { error } = await likeComment(commentId, challengeId);
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 mt-1">
              <button 
                onClick={() => handleLikeComment(comment.id, challengeId)}
