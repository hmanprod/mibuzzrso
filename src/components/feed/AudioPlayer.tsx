...
// Ensure the canvas' internal pixel buffer matches its displayed size for crisp rendering
const resizeCanvas = (canvas: HTMLCanvasElement) => {
  ...
  if (ctx) {
    ctx.resetTransform?.(); // supported in most modern browsers
    ctx.scale(dpr, dpr);
  }
};
...
const drawPlaceholderWaveform = (canvas: HTMLCanvasElement) => {
  ...
  for (let i = 0; i < bars; i++) {
    // Smooth random by averaging with previous value for more natural shape
    const rnd = (Math.random() + prev) / 2;
    prev = rnd;
    const amplitude = 0.15 + rnd * 0.85; // keep some minimum height
    const barHeight = amplitude * (height / 2);
    const x = i * (barWidth + gap);
    const y = (height - barHeight * 2) / 2; // center vertically
...
const AudioPlayer = forwardRef<AudioPlayerRef, AudioPlayerProps>(
  ({ audioUrl, audioDuration, mediaId, postId, coverUrl, comments, onTimeUpdate, downloadable }, ref) => {
    ...
    // Précharger l'audio pour le waveform
  // Dessiner un placeholder dès le montage pour éviter un canvas vide
  useEffect(() => {
