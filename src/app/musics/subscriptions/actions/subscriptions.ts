...
export async function getSubscriptionsMedia(limit = 12, page = 1): Promise<MediaResponse> {
  try {
    ...
    // Get subscribed media with likes
    const { data: mediaData, error } = await supabase
      .rpc('get_media_subscribed', {
    ...
    console.error('Error in getSubscriptionsMedia:', error);
    return {
      media: [],
      total: 0,
      hasMore: false,
