...
import { searchSubscriptionMedias } from '@/app/musics/subscriptions/actions/media';
...
export default function SearchSubscriptionsPage() {
  ...
  const loadMedia = useCallback(async (isInitial: boolean = true) => {
    ...
    try {
      ...
      const result = await searchSubscriptionMedias(
        isInitial ? 1 : page,
        ITEMS_PER_PAGE,
        query
      );
  ...
  if (!query) {
    return (
      <div className="max-w-5xl mx-auto py-8 px-4 space-y-8">
        <h1 className="text-2xl font-semibold">Recherche dans les abonnements</h1>
