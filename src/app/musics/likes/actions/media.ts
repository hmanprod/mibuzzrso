...
export async function searchLikedMedia(
  page: number,
  itemsPerPage: number,
  query: string
  ...
  try {
    ...
    const { data: mediaData, error } = await supabase
      .rpc('get_liked_media_with_keywords', {
    ...
    if (error) {
      console.error('Error searching liked medias:', error);
    ...
    console.error('Error in searchLikedMedia:', err);
